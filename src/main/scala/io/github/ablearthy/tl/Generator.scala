...
object Generator {
  ...
  def generateTypes(tlFile: File, basePath: File): Seq[File] = {
    ...
    val f = writeFunctions(
      gen,
      result.functions,
      basePath.toPath.resolve("functions")
    )
    t ++ f
  }
  ...
  private def writeTypes(gen: Gen, basePath: Path): Seq[File] = {
    ...
    sources.foreach { funit =>
      ...
      val source: Source =
        source"package $pkgName { ..$imports; ..${funit.source} }"
    ...
    val implicitSource =
      source"package $pkgName { ..$implicitImports; $objEnc; $objDec }"
  ...
  private def writeFunctions(
    ...
    val implicitSource =
      source"package $pkgName { ..$implicitImports; $objEnc }"
    val implicitsPath = basePath.resolve("implicits.scala")
    Files.writeString(implicitsPath, implicitSource.toString)
